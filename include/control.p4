...
//-----------------------------------------------------
action set_next_stage(next_stage, need_condition) {
	modify_field(STAGE_ID, next_stage);
	modify_field(vdp_metadata.need_condition,);
}
...
action set_load_from_header(right1, mask1) {
    modify_field(vdp_metadata.load_match,  
		((HDR >> right1) & mask1));

	
}
action set_load_from_meta(right1, mask1) {
    modify_field(vdp_metadata.load_match,  
		((META >> right1) & mask1));
}
action set_load_from_standard(right1, mask1) {
    modify_field(vdp_metadata.load_match,  
		((standard_metadata.ingress_port >> right1) & mask1));
}
