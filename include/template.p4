...
table table_match_locate_##X {                                				\
	reads {																	\
		vdp_metadata.inst_id : exact ;                                		\
		vdp_metadata.stage_id : exact ;                                     \
        vdp_metadata.branch	: exact ; 	                                    \
	}                                                       				\
	actions {																\
		set_load_from_header; 												\ 
		set_load_from_standard; 												\
		set_load_from_meta; 														\
	}                														\
}                                                                                 \
table table_match_result_##X {                                              \
	reads {                                                                 \
		vdp_metadata.inst_id : exact ;                                		\
		vdp_metadata.stage_id : exact ;                                  	\
		vdp_metadata.load_match : ternary ;                             	\
	}                                                                       \
	actions { 																\
		set_match_result; 												\
		set_action_id;														\
...
control conditional_##X {		    										\
	...
	if (context_metadata.left_expr < context_metadata.right_expr) {			\
		set_branch_result(BRANCH_LESS);											\
	} 																		\
	else if(context_metadata.left_expr > context_metadata.right_expr) {		\
		set_branch_result(BRANCH_GREATER);											    \	
	} 																		\
	else {																	\
		set_branch_result(BRANCH_EQUAL);													\
	}
    apply(table_branch_##X);		                                        \
}																			\
...
control execute_do_##X {												\
	...
	if ((ACTION_BITMAP & BIT_MASK_NEXT_LEVEL) != 0) {                   \
